...
type imagePackages map[string]*Package
...
type Resolver struct {
	...
	imagesCacheLock sync.RWMutex
	imagesCache     *simplelru.LRU[string, imagePackages]
	...
	imagesCacheHit        *atomic.Uint64
	imagesCacheMiss       *atomic.Uint64
...
func NewSBOMResolver(c *config.RuntimeSecurityConfig, statsdClient statsd.ClientInterface) (*Resolver, error) {
	...
	imagesCache, err := simplelru.NewLRU[string, imagePackages](c.SBOMResolverWorkloadsCacheSize, nil)
	if err != nil {
	...
	resolver := &Resolver{
		...
		imagesCacheHit:        atomic.NewUint64(0),
		imagesCacheMiss:       atomic.NewUint64(0),
	}
...
func (r *Resolver) SendStats() error {
	...
	r.imagesCacheLock.RLock()
	defer r.imagesCacheLock.RUnlock()
	if val := float64(r.imagesCache.Len()); val > 0 {
	...
	if val := int64(r.imagesCacheHit.Swap(0)); val > 0 {
	...
	if val := int64(r.imagesCacheMiss.Swap(0)); val > 0 {
